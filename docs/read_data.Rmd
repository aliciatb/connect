...
Find a dataset on a Socrata site, like https://www.dallasopendata.com/
...
### Here's an example: Unemployment Rate by Age Groups in California


```{r rReadrExample}
# public dataset example requiring no authentication
unemp_age <- read_csv("https://data.edd.ca.gov/api/views/bcij-5wym/rows.csv?accessType=DOWNLOAD&bom=true&format=true") %>% 
  mutate(Date = lubridate::mdy(Date))
str(unemp_age)
```

## Try using readr with the CSV endpoint! Part 1

First up, use readr with the Socrata dataset you chose earlier (the one you used with RSocrata)
1. Right click on the CSV or CSV for Excel button on the Socrata dataset and copy that link
2. Paste it in the code chunk and use readr::read_csv()

```{r CSVEndpoint,eval=F}

```

## Try using readr with the CSV endpoint! Part 2

Now, use the Local Area Unemployment Statistics endpoint provided with readr
Feel free to use str(), head(), etc to get to know the data a little. 

```{r readr LAUS, eval=F}
## "CSV for Excel" option on a dataset's Primer page provides this endpoint
readr_laus_csv <- "https://data.edd.ca.gov/api/views/e6gw-gvii/rows.csv?accessType=DOWNLOAD&bom=true&format=true"
 
readr_laus_data <- 

```

The Date column is clearly a date, but is listed as a Character data type. Run the code chunk below to fix it. 
This uses the dplyr and lubridate packages, part of the tidyverse!

```{r LAUSDate}
readr_laus <- readr_laus %>% dplyr::mutate(Date = lubridate::mdy(Date))
str(readr_laus)
```
...
- copy the SODA json endpoint
- add a limit parameter to grab as many rows as you need (be kind to the hotel wifi)
