...
Find a dataset on a Socrata site, like https://www.dallasopendata.com/, and use RSocrata::read_socrata() to read it in.
...
2. Use readr::read_csv() on that endpoint, like above (Leave the col_types bit out).
...
Now, use readr with the Local Area Unemployment Statistics endpoint provided
...
The Date column is clearly a date, but we didn't parse it as that type. Run the code chunk below to fix it. 
This uses the dplyr and lubridate packages, part of the tidyverse! (just one way to parse it, readr can parse too, like in the example above).

```{r LAUSDate}
readr_laus_data <- readr_laus_data %>% dplyr::mutate(Date = lubridate::mdy(Date))
str(readr_laus_data)
```
